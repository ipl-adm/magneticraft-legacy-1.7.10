...
public class TileWindTurbine extends TileConductorLow implements IInventoryManaged,IGuiSync,IEfficient,IProductor{
	...
	private boolean hasSpace(IWindTurbine item) {
		...
		for(int y=-h+1;y<h;y++){
			for(int v=-l+1;v<l;v++){
				int i = 0,k = 0;
				switch(facing){
				case NORTH: k=-1; i = v; break;
				case SOUTH: k=1; i = v; break;
				case WEST: k=v; i = -1; break;
				case EAST: k=v; i = 1; break;
	...
	public void sendGUINetworkData(Container cont, ICrafting craft) {
		...
		craft.sendProgressBarUpdate(cont, 2, (int) production);
		craft.sendProgressBarUpdate(cont, 3, (int) productionPerSecond);
	}
	...
	public void getGUINetworkData(int id, int value) {
		...
		if(id == 2)production = value;
		if(id == 3)productionPerSecond = value;
	}
