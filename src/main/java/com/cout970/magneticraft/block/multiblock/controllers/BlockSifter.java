...
public class BlockSifter extends BlockMg implements MB_ControlBlock {
    ...
    public boolean onBlockActivated(World w, int x, int y, int z, EntityPlayer p, int side, float p_149727_7_, float p_149727_8_, float p_149727_9_) {
        ...
        if (t instanceof TileSifter) {
            if (!((TileSifter) t).isActive()) {
                if (!w.isRemote)
                    MB_Watcher.watchStructure(w, new VecInt(x, y, z), MB_Register.getMBbyID(MB_Register.ID_SIFTER), getDirection(w, new VecInt(x, y, z)), p);
                else
                    ((TileSifter) t).drawCounter = 200;
            } else {
    ...
    public Multiblock getStructure() {
        return MB_Register.getMBbyID(MB_Register.ID_SIFTER);
    }
