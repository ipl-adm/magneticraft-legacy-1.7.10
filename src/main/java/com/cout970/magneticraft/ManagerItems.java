...
import com.cout970.magneticraft.items.ItemGuideBook;
...
public class ManagerItems {
	...
	public static String[] oreNames = new String[] { "Iron", "Gold", "Copper", "Tin", "Tungsten", "Lead", "Silver",
			"Uranium", "Thorium", "Nickel", "Ardite", "Cobalt", "Zinc", "Aluminium", "Platinum", "Titanium", "Osmium",
			"Bismuth", "Chromium", "Mithril", "Lithium", "Iridium", "Manganese" };
	
	public static String[][] extraNames = new String[][] { { "Nickel", "Aluminium" }, { "Copper", "Silver" },
			{ "Gold", "Iron" }, { "Iron", "Silver" }, { "Iron", "Aluminium" }, { "Silver", "Thorium" },
			{ "Lead", "Copper" }, { "Thorium", "Plutonium" }, { "Uranium", "Plutonium" }, { "Iron", "Zinc" },
			{ "Cobalt", null }, { "Ardite", null }, { "Iron", "Nickel" }, { "Iron", "Titanium" },
			{ "Nickel", "Silver" }, { "Iron", "Nickel" }, { null, "Iron" }, { null, "Zinc" }, { null, "Nickel" },
			{ null, "Zinc" }, { null, "Iron" }, { null, "Iron" }, { "Iron", null } };
			
	public static List<ItemProduct> chunks = new ArrayList<ItemProduct>();
	// public static List<ItemProduct> chunks_clean = new
	// ArrayList<ItemProduct>();
	public static List<ItemProduct> rubble = new ArrayList<ItemProduct>();
	// public static List<ItemProduct> rubble_clean = new
	// ArrayList<ItemProduct>();
	public static List<ItemProduct> pebbles = new ArrayList<ItemProduct>();
	// public static List<ItemProduct> pebbles_clean = new
	// ArrayList<ItemProduct>();
	...
	private static void registerProducts() {
		for (String name : oreNames) {
			...
			// chunks_clean.add(new
			// ItemProduct("chunk_clean_"+name.toLowerCase(), "cleanChunk"+name,
			// name));
			rubble.add(new ItemProduct("rubble_" + name.toLowerCase(Locale.US), "rubble" + name, name));
			// rubble_clean.add(new
			// ItemProduct("rubble_clean_"+name.toLowerCase(),
			// "cleanRubble"+name, name));
			pebbles.add(new ItemProduct("pebbles_" + name.toLowerCase(Locale.US), "pebbles" + name, name));
			// pebbles_clean.add(new
			// ItemProduct("pebbles_clean_"+name.toLowerCase(),
			// "cleanPebbles"+name, name));
		...
		for (ItemProduct i : chunks)
			addItem(i, i.getBaseName() + " Chunk");
		// for(ItemProduct i : chunks_clean)addItem(i, i.getBaseName()+" Clean
		// Chunk");
		for (ItemProduct i : rubble)
			addItem(i, i.getBaseName() + " Rubble");
		// for(ItemProduct i : rubble_clean)addItem(i, i.getBaseName()+" Clean
		// Rubble");
		for (ItemProduct i : pebbles)
			addItem(i, i.getBaseName() + " Pebbles");
		// for(ItemProduct i : pebbles_clean)addItem(i, i.getBaseName()+" Clean
		// Pebbles");
		for (ItemProduct i : dust)
			addItem(i, i.getBaseName() + " Dust");
		for (ItemProduct i : sinterClump)
			addItem(i, i.getBaseName() + " Sinter Clump");
		for (ItemProduct i : sinter)
			addItem(i, i.getBaseName() + " Sinter");
	}
