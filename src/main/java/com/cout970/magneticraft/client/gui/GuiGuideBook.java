			...
			if(Magneticraft.DEBUG){
				currentPage.gadgets.stream().filter(holder -> holder != null).forEach(holder -> {
					...
					if (comp != null) {
						...
						drawRect(box.getMinX(), box.getMinY(), box.getMinX()+1, box.getMaxY(), color);
						drawRect(box.getMinX(), box.getMaxY(), box.getMaxX(), box.getMaxY()+1, color);
						drawRect(box.getMinX(), box.getMinY(), box.getMaxX(), box.getMinY()+1, color);
						drawRect(box.getMaxX(), box.getMinY(), box.getMaxX()+1, box.getMaxY(), color);
					}
				});
			}
		}
		...
		if (currentPage != null) {
			currentPage.gadgets.stream().filter(holder -> holder != null).forEach(holder -> {
				...
				if (comp != null) {
					...
					if(isIn(comp.getBox().copy().translate(xStart, yStart), x, y)){
						selectedComp = comp;
					}
				}
			});
